...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
		...
		shape = cv2_img.shape
		width_block = shape[1]/10
		img = cv2.GaussianBlur(cv2_img,(9,9),0)
		gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
		color = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
		gray2 = cv2.cvtColor(color, cv2.COLOR_BGR2GRAY)

		cropped_image = gray2[shape[0]-80:shape[0], 0:shape[1]]

		threshold = 150
		_, img_bin = cv2.threshold(cropped_image, threshold, 255, cv2.THRESH_BINARY_INV)

		im, contours,hierarchy = cv2.findContours(img_bin,cv2.RETR_TREE,
											cv2.CHAIN_APPROX_SIMPLE)
		#with_contours = cv2.drawContours(cv2_img,contours,-1,(0,255,0),3)

		#cv2.imshow('camera', img_bin)
		...
		for c in contours:
			...
			if M["m00"] != 0:
				...
				for i in range(0, len(state)-2):
					if(cX > width_block*i and cX<width_block*(i+1)):
						state[i] = 1
						#cv2.circle(cv2_img,((width_block*i + width_block*(i+1))/2, 750), 15, (0,200,200), -1)
				else: 
					state[9] = 1
					#cv2.circle(cv2_img,((width_block*9 + shape[1])/2, 750), 15, (0,200,200), -1)
